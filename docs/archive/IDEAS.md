...
I want to move this towards center, allow it to go down by relative 5% per additional unopposed vote, with 5s min. (wonder if simple % scaling would have been smarter than shannon entropy we used for existing timer implementation?) new absolute min becomes significantly shorter if completely unopposed.
